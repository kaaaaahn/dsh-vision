...
async function detectEnv(ctx) {
  ...
  // 模型能力 patch 状态（上传图片预检依赖它；用 fs 检测，避免 shell 路径转义问题）
  try {
    const text = await fsp.readFile(deepseekAdapterFile(), 'utf8')
    env.modelPatch = text.includes('inputModalities: ["text", "image"]')
  } catch (e) {
    env.modelPatch = false
    env.modelPatchError = String((e && e.message) || e)
  }
  return env
}
...
// state: idle | running | done | failed；patch 失败单独记录，不阻塞整体就绪
const provision = { state: 'idle', steps: [], error: null, patchError: null, startedAt: null, finishedAt: null }
...
async function autoProvision(ctx) {
  ...
  provision.patchError = null
  try {
    const env = await detectEnv(ctx)
    if (env.error) throw new Error(env.error)
    console.log('[zenk-vision] autoProvision: ollama=' + (env.ollamaBinary || '无') + ' svc=' + (env.ollamaService ? 'ok' : 'down') + ' model=' + (env.modelReady ? 'ok' : 'missing:' + env.recommended) + ' patch=' + (env.modelPatch ? 'ok' : 'missing'))
    // 快速路径：ollama + 模型就绪即可（patch 失败只影响上传，不阻塞分析能力）
    if (env.ollamaBinary && env.ollamaService && env.modelReady) {
      provision.state = 'done'
      provision.patchError = env.modelPatch ? null : (env.modelPatchError || 'patch 未生效（仅影响上传图片预检）')
...
function provisionHint() {
  switch (provision.state) {
    ...
    case 'done': return '视觉环境已就绪' + (provision.patchError ? '（' + provision.patchError + '）' : '')
